...
export default class Router {
	...
	initializeRoutes() {
		...
		this.app.post('/emojis', async (req, res) => {
			...
			let post = await this.addCurrentUserEmojsToPosts(emojis[1], req.userId)
			console.log(post[0])
			reactionsSocket.emit(`emojis_count_${emojis[0].post_id}`, post[0]);
			res.json(emojis[0])
		})
		this.app.delete('/emojis', async (req, res) => {
			const emojis = await EmojisController.deleteEmoji(req.body, req.userId)
			const post = await this.addCurrentUserEmojsToPosts(emojis[1], req.userId)
			console.log(post[0])
			reactionsSocket.emit(`emojis_count_${post[0]._doc._id}`, post[0]);
			res.json(emojis)
		})
